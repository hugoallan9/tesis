...
\indent Entonces, $\alpha \in \ker(\omega^*)$ si y sólo si $ \sum_jr_{ij} = 0 $ para cada calor de $i$. Entonces si se tiene un $\alpha \in \ker(w^*)$ se puede escribir
\begin{eqnarray*}
...
\indent Es claro que dicha función existe, ya que $G$ es finito por la segunda condición y se cumple que $\frac{1}{|G|} < \infty$ por la tercera. Se desea probar que $\pi ^ * $ es un $RG~\mbox{-homomorfismo}$  tal que $(\pi ^*)^2 = \pi ^* $ y $M = Im(\pi ^*)$, lo cual se muestra en dos partes a continuación:\\
...
\newItem Sobreyectividad y composición: nótese que $gm \in M$ ya que $M$ es un \nopagebreak[0] $\modulo{RG}$, así que $\pi (gm) = gm$ y por lo tanto 
