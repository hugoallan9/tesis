...
\begin{figure}
\caption{\quad \textbf{Diagrama conmutativo para la proposición \ref{up}.}}
\[\xymatrix { G \ar[r]^f 
\ar[d]_i & A\\
RG \ar@{--}[ru]_{f^*} & }\]
\caption*{Elaboración: fuente propia con paquete \textbf{xymatrix} para computadora.}
...
Por último si $H \lhd G$ entonces para todo $g$ elemento de $G$ se cumple que $gHg^{-1} = H$ de donde $g\hat{H}g^{-1} = \hat{H}$ de donde se concluye inmediatamente que $\hat{H}g = g\hat{H}$ lo cual prueba que $\hat{H}$ es central en $RG$ y de allí se sigue fácilmente la conclusión. \qedhere
\end{proof}
...
Por lo anteriormente expuesto se sigue directamente que $\ker(\pi^*)$ es un $RG$-submódulo tal que $RG = M \oplus ker(\pi^*)$
...
\section{Grupo-Algebras de grupos abelianos}\label{sec:codigos}
