...
\usepackage[round,colon]{bibthesis}
\usepackage{color, enumerate, indentfirst}
...
%Paquete para el manejo del Indic
...
\patchcmd{\caption@@@make}% <cmd>
  {\ifcaption@star}% <search>
    {\ifcaption@star\normalfont\footnotesize}% <replace>
    {}{}% <success><failure>
\makeatother
...
\setlength{\LTcapwidth}{0in}
 %<------------- parametros para tablas largas
\pretolerance=2000
\tolerance=3000 %<- para no separar las palabras
\def\particulartemplate#1{
  \thispagestyle{empty}
  \begin{tikzpicture}[remember picture]
    \draw let \p1 = (current page.west), \p2 = (current page.east) in
      node[minimum width=\x2-\x1, minimum height=0.5cm, draw, rectangle, fill=blue!20, anchor=north west, align=left, text width=\x2-\x1] at ($(current page.north west)$) {\Large\bfseries \quad #1};
  \end{tikzpicture}
}
...
\setlength{\cftfignumwidth}{0.61cm}
...
\cftsetpnumwidth{2em}
%Para cambiar el tamaño de letra al indice (12pts)
\renewcommand\cftchapfont{\normalfont}
...
\setlength\listparindent{0cm}
\setlength\leftmargin{1.4cm} % Distancia entre borde y número
\setlength{\parsep}{\baselineskip}% Separación entre elementos
}
}{
\end{list}
}

\newenvironment{centeredList}{
\begin{list}
{\centering\arabic{ContadorLista}.}
{
\usecounter{ContadorLista}
\setlength{\labelsep}{2.125cm}% Separación entre número y texto
\setlength{\labelwidth}{1cm}
\setlength\listparindent{0cm}
\setlength\leftmargin{2.4cm} % Distancia entre borde y número
\setlength{\parsep}{.0cm}% Separación entre elementos
}
}{
\end{list}
...
\newcommand{\modulo}[1]{#1\mbox{-módulo}}
\newcommand{\newItem}{\item[$\bullet$]}
...
\setstretch{1.45}
\include{Datos}
\captionsetup[figure]{labelformat=simple, labelsep=period,textfont=bf}
\include{portada}
%Para incluir cartas
\newpage
\thispagestyle{empty}
\mbox{}
\includepdf{Cartas/cartaMayra.pdf}
\newpage
\thispagestyle{empty}
\mbox{}
\includepdf{Cartas/aprobacionSergioM.pdf}
\newpage
\thispagestyle{empty}
\mbox{}
\includepdf{Cartas/cartaAsesor2.pdf}
\newpage
\thispagestyle{empty}
\mbox{}
